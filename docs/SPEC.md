...
- **Redirect-URI validation is fail-closed (normative):** `validateRedirectUri` throws on malformed/disallowed input; `matchesRedirectPattern` returns `false` on parse failure. The DCR store's `redirectUriPatterns` is the authoritative allowlist for client redirect URIs — XSUAA only validates this server's own `/oauth/callback`. It is not a mirror of `xs-security.json`, which cannot hold custom schemes (`cursor://`, `vscode://` are rejected by XSUAA, see arc-mcp/arc-1#812). Consumers MUST keep it as narrow as their clients need.
